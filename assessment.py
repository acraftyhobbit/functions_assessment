...
def home_town(town_name):
    """ Takes a town name and campares it to see if it your home town
    """
    if town_name.lower() == "san antonio": #lower cased the argument because not sure on datat formating 
        return True
    else:
        return False
...
def full_name(first_name, last_name):
    """  takes a first and last name as arguments and returns the two names in one string.
    """
    return "{} {}".format(first_name, last_name)


def greetings(town_name,  first_name, last_name):
    """ Takes the functions home_town and full_name and returns a string with the information for a greeting
    """
    if home_town(town_name=town_name) == True:
...
def is_berry(fruit):
    ...
    """
# A boolean refers to true or false statements
    if fruit in ["strawberry", "raspberry", "blackberry"]: 
#in means one of the following doesn't have to be all
        return True
    else:
        return False
...
def shipping_cost(fruit):
    ...
    """
# follow same format as calling function in part 1c
    if is_berry(fruit=fruit) == True:
        return 0
    else:
        return 5
...
def calculate_price(base_price, state, tax=0.05):
   ...
   #addtional fees on to of the tax
    if state == "CA":
        total_price *= 1.03
    elif state == "PA":
        total_price += 2.00
...
def creates_new_list(lst,*args):
    """Takes in a list and any number of additional arguments, appends them to the list, and returns the entire list.
    """
    return lst + args #list addition
# you can embed functions insides of function
# Note only other function can use _replicate_by_three other than outer() 
def outer(word):
    """The outer function will take in a word. Then inner function will multiply that word by 3.Then, the outer function will call the inner function. Print the output as a tuple.

    >>> outer("Balloonicorn")
    ('Balloonicorn', 'BalloonicornBalloonicornBalloonicorn')"""

    def __replicate_by_three__(word):
        return word * 3
# tested in ipython if you can multiply a number ans: yes
    return (word, __replicate_by_three__(word))
###############################################################################
